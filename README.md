...
`FindOneAndUpdate`, `FindOneAndReplace`, and `FindOneAndDelete` find a document, apply the change, and return the document — all atomically.
...
// Swap the entire document; returns the document before replacement (default)
doc, err := col.FindOneAndReplace(ctx, filter, []byte(`{"name":"Alice","age":31}`))

// Returns the document after replacement
doc, err := col.FindOneAndReplace(ctx, filter, []byte(`{"name":"Alice","age":31}`), mongopher.WithReturnAfter())

// Returns the deleted document
doc, err := col.FindOneAndDelete(ctx, filter)
```

All three return `ErrNoDocuments` when no document matches the filter.
